...
// "YYYY-MM-DD" for today in UTC — matches the day boundary utcMidnightMs() uses,
// so it lines up with what the date picker (and selectedMidnightMs below) expect.
...
// UTC midnight of the day the board is currently showing: the user-picked date
// (state.dateStr) if one is set, otherwise the real "today".
function selectedMidnightMs() {
  if (state.dateStr) {
    const [y, m, d] = state.dateStr.split("-").map(Number);
    return Date.UTC(y, m - 1, d);
  }
  return utcMidnightMs(new Date());
}
...
function render() {
  ...
  const midnight = selectedMidnightMs();
...
datePicker.addEventListener("change", () => {
  state.dateStr = datePicker.value || null;
  render();
});
